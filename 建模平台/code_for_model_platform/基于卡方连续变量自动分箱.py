...
#基于卡方分层
def moto_binning(data_0, avg6_shouru: str, y:str):
    """data_1是数据集,
    ...
    xx_2 = [np.percentile(data_1[avg6_shouru].dropna(), x) for x in xx] #按比例算分位点
    
    output_1 = pd.Series()
    for x in set(xx_2):
        data_1['new_var'] = (data_1[avg6_shouru] <= x).astype(int)
        pp = pd.crosstab(data_1['%s' % y],data_1['new_var'])
        dd = np.array([list(pp.iloc[0]),list(pp.iloc[1])])
        pppp = chi2_contingency(dd)
        output_1['<=%s' % x] = pppp[0]
        new_var_dict['<=%s' % x] = (data_1[avg6_shouru] <= x).astype(int)
    ...
    for i in range(1, 45):
        for j in range(i+5, 50):
            i_1 = np.percentile(data_1[avg6_shouru].dropna(), i*2)
            j_1 = np.percentile(data_1[avg6_shouru].dropna(), j*2)
            data_1['new_var_2'] = ((data_1[avg6_shouru] > i_1) & (data_1[avg6_shouru]<=j_1)).astype(int)
            ...
            new_var_dict['>%s and <=%s'%(i_1, j_1)
             ] = ((data_1['%s'%avg6_shouru] > i_1) & (data_1['%s'%avg6_shouru] <= j_1)).astype(int)
